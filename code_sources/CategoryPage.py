...
def extract_transform_load_one_book_infos(book, nameFile, category_name, pictures_directory, session):
    ...
    BookPage.extract_transform_load_all_infos_in_csv(book_page_url, nameFile, category_name, pictures_directory, session)
...
def check_if_there_is_next_page(category_soup, product_category_url):
    ...
    if next_page:
        ...
        return BeautifulSoup(next_page_html.content, 'lxml')
    else:
        return None


def extract_transform_load_all_books_infos_from_category(product_category_url, file_csv_name, category_name,
                                                         pictures_directory, session):
    product_category_html = session.get(product_category_url)
    category_soup = BeautifulSoup(product_category_html.content, 'lxml')
    while (category_soup):
        ...
        for book_article in all_books_category_article:
            extract_transform_load_one_book_infos(book_article, file_csv_name, category_name, pictures_directory, session)
